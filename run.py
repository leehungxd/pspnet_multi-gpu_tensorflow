...
def get_pid():
    ...
    for proc in psutil.process_iter():
        if proc.name() == 'python' and proc.name() == '/home/lihang/projects/tf-code/PSPNet':
           pid = proc.pid
    return pid
...
def train_and_eval():
    ...
    show_period = FLAGS.train_max_iter // 8925
    ...
    while step < max_iter + 1:
        ...
        if step % show_period == 0:
            ...
            print('\033[1;36m [%s %s %s] Iteration %d, lr = %f,\n [%s %s %s] wd_mode = %d, wd_rate = %f, wd_rate_2 = %f \033[0m'
                  % (str(datetime.datetime.now()), str(os.getpid()), 'run.py:267', step, lrn_rate,
                     str(datetime.datetime.now()), str(os.getpid()), 'run.py:268', FLAGS.weight_decay_mode, wd_rate, wd_rate2))
            for i, tag in enumerate(tags.split(',')):
                # print(tag, '\033[1;32m = \033[0m', merged_losses[i], end=', ')
                print('\033[1;32m [%s %s %s] %s = %f, \033[0m'%(str(datetime.datetime.now()), str(os.getpid()), 'run.py:269', tag, merged_losses[i]))
            # print('')
            print('\033[1;31m [%s %s %s] Estimated time left: %.2f hours. %d/%d\033[0m' % (str(datetime.datetime.now()), str(os.getpid()), 'run.py:273',
        ...
        if step % snapshot == 0 or step == max_iter:
            ...
            eval_loss = 0.0
            for i in trange(len(eval_image_reader.image_list)):
                ...
                # eval_losses = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=prediction, labels=one_label)
                # eval_loss += sess.run(tf.reduce_sum(eval_losses))
            # eval_loss = eval_loss / len(eval_image_reader.image_list)
