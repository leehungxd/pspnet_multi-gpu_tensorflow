...
def compute_iou(confusion_matrix):
    ...
    print(' [%s %s %s] [IoUs]: '%(str(datetime.datetime.now()), str(os.getpid()), 'helper.py:248'), end='\n')
    for i in range(confusion_matrix.shape[0]):
        print(' [%s %s %s] %d %.2f' % (str(datetime.datetime.now()), str(os.getpid()), 'helper.py:269', i, iou[i]*100), '& ', end='\n')
    print(' [%s %s %s] [mIoU]: %.2f' % (str(datetime.datetime.now()), str(os.getpid()), 'helper.py:251', np.mean(iou)*100), '\\\\')
    return np.mean(iou)
